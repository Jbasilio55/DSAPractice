...
public class SplitArrayLargestSum {
    ...
    public static int splitArray(int[] nums, int k) {
        ...
        for(int i = 0; i < nums.length; i++){
            start = Math.max(start, nums[i]); // after the loop is done, it will contain the max element in the given array.
            end += nums[i];
        }
        ...
        while(start < end){
            ...
            //calculate how many pieces can you divide this max sum
            int sum = 0;
            int pieces = 1;
            for(int num : nums){
                if(sum + num > mid){
                    //you cannot add this in this sub-array, make the new one
